...
public class SerialPortService extends Service{
	...
	public void onDestroy() {
        ...
        mInputStream = null;
        mOutputStream = null;
        if (mReadDataFromSpThread != null || !mReadDataFromSpThread.isInterrupted()) {
            Trace.i("SerialPortService mReadDataFromSpThread interrupt");
            mReadDataFromSpThread.interrupt();
        }
        if (mSendDataToSpThread != null || !mSendDataToSpThread.isInterrupted()) {
    ...
    public SerialPortWriteTask getTask(final String hexString){
        ...
        SerialPortWriteTask task = new SerialPortWriteTask() {
            ...
            public void excute() {
                ...
                Trace.d("send byte to serial prot : " + BytesUtil.bytesToHexString(packet));
                try {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case Contacts.MSG_DATA:
                    ...
                    try {
                        for (int i = 0; i < N; i++)
                            mCallbacks.getBroadcastItem(i).readDataFromServer(mPacket);
    ...
    private final ISerialPortService.Stub mBinder = new ISerialPortService.Stub() {
        ...
        @Override
        public void addClient(ISerialPortCallback client) throws RemoteException {
        ...
        @Override
        public void removeCliend(ISerialPortCallback client) throws RemoteException {
            if(client != null){
